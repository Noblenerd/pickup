		...
		<div class="tab-content">
			<div class="tab-pane container" id="tabb1"> 
				<br>
						...
						$result = mysqli_query($con, "SELECT * FROM orders WHERE Username='$user' ORDER BY ID DESC") or die(mysqli_error($con));
			...
			<div class="tab-pane container active" id="tabb2"> 
				...
				<li class="list-group-item bg-white">
					<?php
						...
						$result = mysqli_query($con, "SELECT * FROM members WHERE Username='$user'") or die(mysqli_error($con));
						...
						<p align="right"><button class="btn btn-danger btn-lg" type="submit" name="submit_upd"> Update <span class="fas fa-cart"></span></button></p>
							...
							//$password = check($_POST['password']);
								...
								if(empty($_FILES["image"]["tmp_name"])) {
									mysqli_query($con, "UPDATE members SET Phone='$phone',Address='$address' WHERE Username='$user'") or die(mysqli_error($con));    
									...
									if(move_uploaded_file($_FILES["image"]["tmp_name"], $path)) {
										...
										mysqli_query($con, "UPDATE members SET Phone='$phone',Address='$address',Image='$path' WHERE Username='$user'") or die(mysqli_error($con));    
			...
			<div class="tab-pane container active mx-0 px-0" id="tabbb1"> 
				...
				<?php
							...
							if(isset($_POST['submit_hum'])) {
								...
								$daate = $date .' ' .$time;
								$sta = 'Pending';
								$cod = md5(sha1(uniqid('mxp', FALSE)));
								$code = substr($cod, 0, 10);
									...
									if($ext == false) {
										...
										if(in_array(strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION)), $allowed)) {
											...
											if(move_uploaded_file($_FILES["image"]["tmp_name"], $path)) {
												...
												VALUES('','$subject','$source','$destination','$phone','$weight','$comment','$daate','$path','$user','$sta','$code',NOW())") or die(mysqli_error($con));    
