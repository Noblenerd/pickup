			...
			<br><br><ul class="list-group logy">
				...
				<li class="list-group-item">
					<ul class="list-group">
						<?php
							$sta = 'Pending';
							$result = mysql_query("SELECT * FROM orders WHERE Status='$sta' ORDER BY ID DESC") or die(mysql_error());
							while($row = mysql_fetch_array($result)) {
								...
								if(isset($_POST['finalize'])) {
									$stu = 'Completed';
									$title = $_POST['title'];
									mysql_query("UPDATE orders SET Status='$stu' WHERE Subject='$title'") or die(mysql_error());
									echo 'Updated';
								}
								...
								echo '<li class="list-group-item bg-white collapse" id="self' .$row['ID'] .'">
										<ul class="list-group">
											...
											$rt = mysql_query("SELECT * FROM members WHERE Username='$uy'");
