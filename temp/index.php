						...
						<?php
							...
							if(isset($_POST['submit_log'])) {
								if(empty($_POST['username']) || empty($_POST['password'])) {
									...
									$result = mysql_query("SELECT * FROM members WHERE Username='$user'AND Password='$pass'") or die(mysql_error());
							...
							if(isset($_POST['submit_reg'])) {
								if(empty($_POST['name']) || empty($_POST['email']) || empty($_POST['phone']) || empty($_POST['address']) || empty($_POST['username']) || empty($_POST['password'])) {
									...
									if(move_uploaded_file($_FILES["image"]["tmp_name"], $path)) {
										...
										VALUES('','$name','$email','$phone','$address','$path','$username','$password',NOW())") or die(mysql_error());    
